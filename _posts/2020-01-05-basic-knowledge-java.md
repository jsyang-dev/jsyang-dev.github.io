...
- 추상 클래스
  - 상속을 통해 부모 클래스의 기능을 이용, 확장하기 위해 제공되며 다중 상속이 불가능함
- 인터페이스
  - 빈 껍데기 형태로 구현할 메소드를 정의하고, 강제해서 서브클래스들에 같은 동작을 보장하기 위해 제공되며 다중 상속이 가능함
  - 자바 8 부터 인터페이스에도 default 메소드 구현이 가능함
...
- 프로세스
  - 자바에서 예를 들면, 실행중인 프로그램을 종종 프로세스라고 함
- 스레드
  - 프로세스 내에서 실행될 수 있는 여러개의 경량화된 프로세스의 개념
  - 한 프로세스 내 여러 스레드는 프로세스 내의 주소 공간이나 자원들을 대부분 공유하면서 실행됨
  - 프로세스를 여러개 생성하기 보다는 멀티 스레드를 이용하는것이 효율적
- 스레드의 장단점
  - 컨텍스트 스위칭 비용이 감소
  - 프로세스 간 통신보다는 스레드 간 통신이 훨씬 비용이 적음
  - 스레드 간 데이터 공유에 따른 동기화 문제를 해결하기 어렵고 그에 따라 디버깅이 어려워지는 단점 존재
